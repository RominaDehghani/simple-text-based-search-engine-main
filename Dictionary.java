...
public class Dictionary<K extends Comparable<? super K>, V> implements DictionaryInterface<K, V> {
	...
	private int locateIndex(K key) {
		int index = 0;
		while ((index < numOfEntries) && dictionary[index] != null && !(key.equals(dictionary[index].getKey())))
			index++;
		return index;
	}
	...
	public boolean contains(K key, V value) {
		V result = null;
		int keyIndex = locateIndex(key);
		if ((keyIndex < numOfEntries) && key.equals(dictionary[keyIndex].getKey()) && value.equals(dictionary[keyIndex].getValue()))
			return true;
		return false;
	}
